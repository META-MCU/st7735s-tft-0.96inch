...
{
	stc_gpio_cfg_t stcGpioCfg; //定义GPIO配置结构体stcGpioCfg
	
    Sysctrl_SetPeripheralGate(SysctrlPeripheralGpio, TRUE); //打开GPIO外设时钟门控
	
	stcGpioCfg.enDir = GpioDirOut; //端口方向配置：输出
    stcGpioCfg.enPu = GpioPuEnable; //端口上下拉配置：开启上拉开关
    stcGpioCfg.enPd = GpioPdDisable; //端口输出模式配置：关闭下拉开关
	stcGpioCfg.enOD = GpioOdDisable; //GPIO开漏输出关闭
	
	Gpio_Init(LCD_PORT, LCD_PIN_SCK, &stcGpioCfg);
	Gpio_Init(LCD_PORT, LCD_PIN_SDA, &stcGpioCfg);
	Gpio_Init(LCD_PORT, LCD_PIN_RES, &stcGpioCfg);
	Gpio_Init(LCD_PORT, LCD_PIN_A0, &stcGpioCfg);
	Gpio_Init(LCD_PORT, LCD_PIN_CS, &stcGpioCfg);
}
